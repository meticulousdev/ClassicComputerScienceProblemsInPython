...
class GeneticAlgorithm(Generic[C]):
    ...
    def _advanced_pick_tournament(self, num_participants: int) -> Tuple[C, C]:
        participants: List[C] = choices(self._population, k=num_participants)
        last_four: List[C] = nlargest(3, participants, key=self._fitness_key)
        return tuple(nsmallest(2, last_four, key=self._fitness_key))
    ...
    def _reproduce_and_replace(self) -> None:
        ...
        while len(new_population) < len(self._population):
            if self._selection_type == GeneticAlgorithm.SelectionType.ROULETTE:
                ...
                parents = self._advanced_pick_tournament(len(self._population) // 2)
