...
class KMeans(Generic[Point]):
    ...
    def __init__(self, k: int, points: List[Point]) -> None:
        ...
        self._centers: List[Point] = points.copy()
        self._zscore_normalize()
        self._clusters: List[KMeans.Cluster] = []

        idx: int = self._centers.index(choice(self._centers))
        center: DataPoint = self._centers.pop(idx)
    ...
    def _choice_next(self, center: DataPoint) -> DataPoint:
        differences: List[float] = [DataPoint.distance(center, point) for point in self._centers]
        probabilities: List[float] = [DataPoint.distance(center, point) / sum(differences)
                                      for point in self._centers]
        idx: int = probabilities.index(max(probabilities))
        return self._centers.pop(idx)
